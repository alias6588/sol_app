...
class MusicPlayerScreen extends StatefulWidget {
  ...
  MusicPlayerScreen({required this.noteEvents});
...
class _MusicPlayerScreenState extends State<MusicPlayerScreen> {
  ...
  Future<void> _playMusic() async {
    ...
    for (var event in widget.noteEvents) {
    ...
    for (int i = 0; i < widget.noteEvents.length; i++) {
      final noteEvent = widget.noteEvents[i];
      final startTimeMs =
          (noteEvent.offset * quarterNoteDurationSec * 1000).round();
      final durationMs =
          (noteEvent.duration * quarterNoteDurationSec * 1000).round();
      ...
      Future.delayed(Duration(milliseconds: startTimeMs), () {
        ...
        setState(() {
          ...
          noteEvent.isPlaying = true;
        });

        print('Note On: ${noteEvent.displayString} at $startTimeMs ms');
        // Play the note(s)
        if (noteEvent.type == 'note') {
          _flutterMidiPro.playNote(
              key: noteEvent.pitch!, sfId: _soundfontId!, velocity: 127);
        } else if (noteEvent.type == 'chord') {
          for (var pitch in noteEvent.pitches!) {
            _flutterMidiPro.playNote(
        ...
        Future.delayed(Duration(milliseconds: durationMs), () {
          ...
          print('Note Off: ${noteEvent.displayString} after $durationMs ms');
          // Stop the note(s)
          if (noteEvent.type == 'note') {
            _flutterMidiPro.stopNote(
                key: noteEvent.pitch!, sfId: _soundfontId!);
          } else if (noteEvent.type == 'chord') {
            for (var pitch in noteEvent.pitches!) {
